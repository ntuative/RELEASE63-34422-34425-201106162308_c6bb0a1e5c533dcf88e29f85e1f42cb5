package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_1268:Dictionary;
      ...
      private var var_284:IRoomObjectSprite;
      ...
      private var var_1269:Number = 1.0;
      
      private var var_664:BitmapData;
      
      private var var_828:ColorTransform;
      
      private var var_1549:ColorTransform;
      
      private var var_1945:Matrix;
      ...
      {
         super();
         this.var_1268 = new Dictionary();
         this._visualization = param1;
         this.var_828 = new ColorTransform();
         this.var_828.alphaMultiplier = 1;
         this.var_1549 = new ColorTransform();
         this.var_1945 = new Matrix();
      ...
      {
         var _loc1_:* = null;
         for each(_loc1_ in this.var_1268)
         {
            _loc1_.dispose();
         }
         this.var_1268 = null;
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_112 = this.var_1268[param1];
         this.var_1017 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1944 >= 0)
         {
            this.var_284 = this._visualization.getSprite(this.var_1944);
            if(this.var_284 && this.var_284.asset)
            {
               if(this.var_284.width <= 1 || this.var_284.height <= 1)
               {
                  return;
               }
               if(this.var_100 == null)
               {
                  this.var_100 = this.var_284.asset.clone();
                  if(this.var_828.alphaMultiplier != 1)
                  {
                     this.var_664 = new BitmapData(this.var_100.width,this.var_100.height,true,4278190080);
                  }
               }
               this.var_2659 = -this.var_284.offsetX;
               this.var_2661 = -this.var_284.offsetY;
               this.var_284.asset = this.var_100;
            }
            if(this.var_100)
            {
            ...
            if(this.var_664)
            {
               this.var_664.fillRect(this.var_664.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_112.y * this.var_1269;
         }
         return 0;
      }
      ...
      {
         if(!this.var_112 || !this.var_284)
         {
            return;
         }
         if(this.var_100 && this.var_284.asset != this.var_100)
         {
            this.var_284.asset = this.var_100;
         }
         if(this.var_1017)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_1017 && this.var_112.hasIgnited)
         {
            this.var_1017 = true;
         }
         _loc8_ = this.var_1165 * this.var_1269;
         this.var_112.update();
         if(this.var_1017)
         {
            if(this.var_112.roomObjectSpriteId >= 0)
            ...
            {
               this.var_100.draw(this.var_664,this.var_1945,this.var_828,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_112.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2659 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1269;
               _loc4_ = this.var_2661 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1269;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1549.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_100.draw(_loc7_,this._translationMatrix,this.var_1549,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2660 = parseInt(param1.@size);
         this.var_1944 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1165 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1269 = this.var_2660 / 64;
         this._blend = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this._blend = Math.min(this._blend,1);
         this.var_828.alphaMultiplier = this._blend;
