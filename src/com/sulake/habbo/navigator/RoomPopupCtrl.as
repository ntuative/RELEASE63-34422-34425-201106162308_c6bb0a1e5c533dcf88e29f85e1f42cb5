package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      private var var_137:GuestRoomData;
      
      private var var_2432:RoomThumbnailRenderer;
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2432 = new RoomThumbnailRenderer(param1);
         this.var_939 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_205 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_205.visible = true;
         Util.hideChildren(this.var_205);
         this.refreshRoomName(this.var_205,this.var_137);
         if(this.var_137.ownerName != "" && this.var_137.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_205,!!this.var_137.event ? this.getEventCtg(this.var_137.categoryId) : this.getRoomCtg(this.var_137.categoryId));
         this.refreshRoomDesc(this.var_205,this.var_137);
         this.refreshExtraCont();
         this.refreshInfo(this.var_205,"trading_allowed",this.var_137.allowTrading);
         this.refreshInfo(this.var_205,"doormode_doorbell",this.var_137.doorMode == 1);
         this.refreshInfo(this.var_205,"doormode_password",this.var_137.doorMode == 2);
         Util.moveChildrenToColumn(this.var_205,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_205.height = Util.getLowestPoint(this.var_205);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_205,this.var_137.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_205.findChildByName("roomowner"));
         if(this.var_137.ownerName != "" && this.var_137.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_205.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_939.refreshTags(_loc1_,this.var_137.tags);
         this.refreshTextWithCaption("rating",_loc1_,"" + this.var_137.score);
         if(this.var_137.event)
         {
            this.refreshTextWithCaption("startedat",_loc1_,"" + this.var_137.eventCreationTime);
         }
         this.var_2432.refreshThumbnail(_loc1_,this.var_137.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_217,Tabs.const_257,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
