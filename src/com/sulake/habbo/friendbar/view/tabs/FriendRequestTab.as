package com.sulake.habbo.friendbar.view.tabs
{
   ...
   public class FriendRequestTab extends FriendEntityTab
   {
      ...
      {
         var _loc3_:* = null;
         var _loc2_:FriendRequestTab = !!var_1492 ? var_1492 : new FriendRequestTab();
         _loc2_.var_163 = false;
         if(_loc2_.friend)
         {
            if(_loc2_.friend.figure != param1.figure)
            {
               _loc3_ = IBitmapWrapperWindow(_loc2_._window.findChildByName(const_312));
               _loc3_.bitmap = var_499.getAvatarFaceBitmap(param1.figure);
            }
         }
      ...
      {
         var _loc6_:* = null;
         var _loc1_:IWindowContainer = WINDOWING.buildFromXML(ASSETS.getAssetByName(const_2033).content as XML) as IWindowContainer;
         ...
         var _loc7_:IRegionWindow = WINDOWING.create("ICON",WindowType.const_362,WindowStyle.const_102,WindowParam.const_31,new Rectangle(0,0,25,25)) as IRegionWindow;
         _loc7_.mouseThreshold = 0;
         var _loc8_:IBitmapWrapperWindow = WINDOWING.create("BITMAP",WindowType.const_431,WindowStyle.const_102,WindowParam.const_33,new Rectangle(0,0,25,25)) as IBitmapWrapperWindow;
         ...
         IItemListWindow(_loc1_.findChildByName(const_379)).addListItemAt(_loc7_,0);
         return _loc1_;
      }
      ...
      private function bubbleEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            ...
            {
               case const_2025:
                  var_348.acceptFriendRequest(var_164.id);
                  break;
               case const_1395:
                  if(selected)
                  {
                  ...
                  break;
               case const_1025:
                  var_348.declineFriendRequest(var_164.id);
            }
         }
         else if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_OVER)
         {
            if(param2.name == const_1025)
            {
         ...
         else if(param1.type == WindowMouseEvent.const_25)
         {
            if(param2.name == const_1025)
            {
               ITextWindow(IWindowContainer(param2).getChildByName(REGION_REJECT_TEXT)).textColor = const_1508;
            }
         }
      }
      ...
      public function avatarImageReady(param1:FriendRequest, param2:BitmapData) : void
      {
         ...
         if(!disposed)
         {
            if(friend)
            {
               ...
               {
                  _loc3_ = _window.findChildByName(const_312) as IBitmapWrapperWindow;
                  if(_loc3_)
                  {
                     _loc3_.bitmap = var_499.getAvatarFaceBitmap(param1.figure);
                  }
