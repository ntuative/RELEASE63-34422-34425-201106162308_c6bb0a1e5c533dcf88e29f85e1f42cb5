package com.sulake.habbo.ui.widget.effects
{
   ...
   public class EffectsWidget extends RoomWidgetBase
   {
      ...
      private static const const_1655:int = 320;
      
      private static const const_1654:int = 48;
      
      private static const const_1656:int = 2;
      ...
      {
         return var_915 as EffectsWidgetHandler;
      }
      ...
      public function open() : void
      {
         ...
         if(!this._view)
         {
            ...
            this._view.x = _loc2_.right + const_1656;
      ...
      public function update() : void
      {
         ...
         {
            _loc1_ = this.var_408.getValue(_loc3_.type) as EffectView;
            if(_loc1_)
            {
               _loc1_.update();
            }
            else
            {
               ...
               this.var_408.add(_loc3_.type,_loc1_);
               this.var_67.addListItem(_loc1_.window);
            }
         }
         _loc4_ = this.var_408.length - 1;
         while(_loc4_ >= 0)
         {
            _loc1_ = this.var_408.getWithIndex(_loc4_);
            if(_loc2_.indexOf(_loc1_.effect) == -1)
            {
               this.var_67.removeListItem(_loc1_.window);
               this.var_408.remove(this.var_408.getKey(_loc4_));
               _loc1_.dispose();
            }
            _loc4_--;
         }
         var _loc5_:int = this.var_67.scrollableRegion.height;
         this.var_67.height = Math.max(Math.min(_loc5_,const_1655),const_1654);
      ...
      {
         this.handler.container.catalog.openCatalogPage(CatalogPageName.const_1169,true);
      }
   }
}
