package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      private var var_1143:IDropMenuWindow;
      
      private var var_561:ITextFieldWindow;
      
      private var var_616:Boolean = true;
      
      private var var_927:ICheckBoxWindow;
      
      private var var_1416:ICheckBoxWindow;
      ...
      public function show() : void
      {
         ...
         this.var_36.connection.send(new GetModeratorRoomInfoMessageComposer(this.var_358));
         Logger.log("BEGINNING TO SHOW: " + this.var_358);
      }
      
      public function getType() : int
      {
         return WindowTracker.const_1793;
      }
      ...
      {
         Logger.log("GOT ROOM INFO: " + param1.flatId + ", " + this.var_358);
         if(param1.flatId != this.var_358)
         {
            Logger.log("NOT THE SAME FLAT: " + param1.flatId + ", " + this.var_358);
            return;
         }
         this._data = param1;
         this.populate();
      ...
      {
         this.var_67 = IItemListWindow(this._frame.findChildByName("list_cont"));
         var _loc1_:IWindow = this._frame.findChildByTag("close");
         _loc1_.procedure = this.onClose;
         this.var_561 = ITextFieldWindow(this._frame.findChildByName("message_input"));
         this.var_561.procedure = this.onInputClick;
         this.var_1143 = IDropMenuWindow(this._frame.findChildByName("msgTemplatesSelect"));
         this.prepareMsgSelect(this.var_1143);
         this.var_1143.procedure = this.onSelectTemplate;
         this.var_927 = ICheckBoxWindow(this._frame.findChildByName("kick_check"));
         this.var_1416 = ICheckBoxWindow(this._frame.findChildByName("lock_check"));
         this.var_1417 = ICheckBoxWindow(this._frame.findChildByName("changename_check"));
      ...
      {
         var _loc3_:IWindowContainer = IWindowContainer(this.var_67.getListItemByName(param2));
         ...
         if(!param1.exists)
         {
            this.disposeItemFromList(this.var_67,_loc3_);
            this.disposeItemFromList(this.var_67,this.var_67.getListItemByName("event_spacing"));
            return;
         }
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_36.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,this._data.flatId),this.var_36,WindowTracker.const_728,this._data.flatId),this._frame,false,false,true);
      }
      ...
      {
         if(this.var_616 || this.var_561.text == "")
         {
            this.var_36.windowManager.alert("Alert","You must input a message to the user",0,this.onAlertClose);
            return;
         }
         var _loc2_:int = this.determineAction(param1,this.var_927.isSelected);
         this.var_36.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_1897,_loc2_,this.var_561.text,"",""));
         if(this.var_1416.isSelected || this.var_1417.isSelected || this.var_927.isSelected)
         {
            this.var_36.connection.send(new ModerateRoomMessageComposer(this._data.flatId,this.var_1416.isSelected,this.var_1417.isSelected,this.var_927.isSelected));
         }
         this.dispose();
      }
      ...
      private function determineAction(param1:Boolean, param2:Boolean) : int
      {
         if(param2)
         {
            return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1790);
         }
         return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1916);
      }
      ...
      private function onSelectTemplate(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc3_:String = this.var_36.initMsg.roomMessageTemplates[this.var_1143.selection];
         if(_loc3_ != null)
         {
            this.var_616 = false;
            this.var_561.text = _loc3_;
         }
      }
   }
}
