package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class SpacesNewCatalogWidget extends ItemGridCatalogWidget implements IDisposable, ICatalogWidget, IItemGrid
   {
      ...
      private var var_447:ISelectorWindow;
      ...
      public function SpacesNewCatalogWidget(param1:IWindowContainer, param2:ISessionDataManager)
      {
         ...
         this.var_716 = [0,0,0];
         this.var_115 = ["group.walls","group.floors","group.views"];
         super(param1,param2);
         this.var_447 = _window.findChildByName("groups") as ISelectorWindow;
         if(this.var_447)
         {
            _loc3_ = 0;
            while(_loc3_ < this.var_447.numSelectables)
            {
               _loc4_ = this.var_447.getSelectableAt(_loc3_);
      ...
      override public function init() : Boolean
      {
         ...
         this.switchCategory(this.var_115[this.var_512]);
      ...
      {
         var _loc2_:int = this.var_716[this.var_512];
         var _loc3_:Offer = this.var_209[this.var_512][_loc2_];
      ...
      {
         var _loc2_:* = null;
         if(param1 > -1 && param1 < var_510.numGridItems)
         {
            _loc2_ = this.var_209[this.var_512][param1];
      ...
      override public function select(param1:IGridItem) : void
      {
         ...
         this.var_716[this.var_512] = (this.var_209[this.var_512] as Array).indexOf(_loc2_);
         this.updateRoomPreview();
      }
      
      private function updateRoomPreview() : void
      {
         var _loc1_:int = this.var_716[0];
         var _loc2_:int = this.var_716[1];
         var _loc3_:int = this.var_716[2];
         var _loc4_:Offer = this.var_209[0][_loc1_];
         var _loc5_:Offer = this.var_209[1][_loc2_];
         var _loc6_:Offer = this.var_209[2][_loc3_];
      ...
      private function createOfferGroups() : Boolean
      {
         ...
         {
            if(!(_loc1_.pricingModel == Offer.const_448 || _loc1_.pricingModel == Offer.const_451))
            {
               continue;
            }
            ...
            if(!(_loc2_.productType == ProductTypeEnum.const_65 || _loc2_.productType == ProductTypeEnum.const_76))
            {
               continue;
            }
            ...
            while(this.var_209.length < this._groupNames.length)
            {
               this.var_209.push(new Array());
            }
            switch(_loc4_)
            {
               case "floor":
                  (this.var_209[_loc5_] as Array).push(_loc1_);
                  break;
               case "wallpaper":
                  (this.var_209[_loc5_] as Array).push(_loc1_);
                  break;
      ...
      private function onSelectGroup(param1:WindowEvent) : void
      {
         ...
         if(_loc2_)
         {
            _loc3_ = this.var_447.getSelectableIndex(_loc2_);
      ...
      private function switchCategory(param1:String) : void
      {
         ...
         if(_loc2_ > -1)
         {
            ...
            if(var_510)
            {
               var_510.destroyGridItems();
            }
            page.replaceOffers(this.var_209[this.var_512],false);
            resetTimer();
            populateItemGrid();
            ...
            _loc3_ = this.var_716[this.var_512];
