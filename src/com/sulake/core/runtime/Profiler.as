package com.sulake.core.runtime
{
   ...
   class Profiler extends Component implements IProfiler
   {
      ...
      private var var_334:Dictionary;
      ...
      {
         this.var_334 = new Dictionary(true);
         super(param1,0,null);
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            for(_loc1_ in this.var_334)
            {
               IDisposable(this.var_334[_loc1_]).dispose();
               delete this.var_334[_loc1_];
            }
            super.dispose();
         }
      }
      ...
      {
         this.var_3041 = getTimer() - this.var_1878;
      ...
      public function getProfilerAgentsInArray() : Array
      {
         ...
         for(_loc3_ in this.var_334)
         {
            _loc2_ = _loc3_ as IUpdateReceiver;
            if(IDisposable(_loc3_).disposed)
            {
               IDisposable(this.var_334[_loc3_]).dispose();
               delete this.var_334[_loc3_];
            }
            else
            {
               _loc1_.push(this.var_334[_loc3_]);
            }
         }
         return _loc1_;
      }
      ...
      {
         var _loc2_:ProfilerAgent = this.var_334[param1];
