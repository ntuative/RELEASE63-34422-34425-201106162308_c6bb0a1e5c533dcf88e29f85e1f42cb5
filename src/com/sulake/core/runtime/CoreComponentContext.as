package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1588:String = "asset-libraries";
      
      private static const const_1587:String = "library";
      
      private static const const_1586:String = "service-libraries";
      
      private static const const_1584:String = "library";
      
      private static const const_1589:String = "component-libraries";
      
      private static const const_1583:String = "library";
      
      private static const const_1591:String = "error_data";
      
      private static const const_1585:String = "error_cat";
      
      private static const const_1590:String = "error_desc";
       
      
      private var var_389:LibraryLoaderQueue;
      
      private var var_845:IEventDispatcher;
      
      private var var_567:uint;
      
      private var var_390:Function;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         var_423.addEventListener(Event.ENTER_FRAME,this.onEnterFrame);
         switch(param2 & 0)
         {
            case Core.const_1300:
               debug("Core; using simple frame update handler");
               this.var_390 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_987:
               debug("Core; using complex frame update handler");
               this.var_390 = this.complexFrameUpdateHandler;
               break;
            case Core.const_916:
               debug("Core; using profiler frame update handler");
               this.var_390 = this.profilerFrameUpdateHandler;
               this.var_222 = new Profiler(this);
               attachComponent(this.var_222,[new IIDProfiler()]);
               var_423.addChild(new ProfilerViewer(this.var_222));
               break;
            case Core.const_523:
               debug("Core; using experimental frame update handler");
               this.var_390 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_390 = this.debugFrameUpdateHandler;
         }
      }
      
      public function getNumberOfFilesPending() : uint
      {
         return this.var_389.length;
      }
      
      public function getNumberOfFilesLoaded() : uint
      {
         return this.var_567 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            if(var_423)
            {
               var_423.removeEventListener(Event.ENTER_FRAME,this.onEnterFrame);
               var_423 = null;
            }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1591,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1585,String(param3));
         ErrorReportStorage.setParameter(const_1590,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_845 = loadingEventDelegate;
         if(this.var_389 == null)
         {
            this.var_389 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1588)[0];
            if(node != null)
            {
               list = node.child(const_1587);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_225,e);
         }
         try
         {
            node = config.child(const_1586)[0];
            if(node != null)
            {
               list = node.child(const_1584);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_225,e);
         }
         try
         {
            node = config.child(const_1589)[0];
            if(node != null)
            {
               list = node.child(const_1583);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_225,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1362);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_845)
         {
            this.var_845.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_845)
         {
            ...
            this.var_845.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_845)
                  {
                     this.var_845.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_567 - this.var_389.length,this.var_567,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_222 ? 0 : this.var_1576 & 0;
         if(_loc3_ == Core.const_523)
         {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_390(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         var maxTimePerFrame:uint = 1000 / var_423.stage.frameRate;
         priority = 0;
         while(priority < CoreComponentContext.const_107)
         {
            t = getTimer();
            ...
            {
               if(this.var_671[priority] < priority)
               {
                  ++this.var_671[priority];
                  skip = true;
               }
            }
            if(!skip)
