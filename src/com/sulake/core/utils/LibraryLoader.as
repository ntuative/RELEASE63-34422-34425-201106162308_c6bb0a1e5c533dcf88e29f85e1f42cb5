package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_2014:uint = 1;
      
      protected static const const_2015:uint = 2;
      
      protected static const const_2017:uint = 4;
      
      protected static const const_2016:uint = 3;
      ...
      public static const const_1249:Boolean = false;
      
      public static const const_1816:int = 2;
      
      private static var var_926:Array = new Array();
      
      private static var var_745:Array = new Array();
      ...
      protected var var_260:URLRequest;
      
      protected var _manifest:XML;
      
      protected var var_652:Class;
      
      protected var var_501:uint = 0;
      ...
      protected var var_1123:Boolean = false;
      
      protected var var_2996:String = "";
      
      protected var var_2994:String = "";
      
      protected var var_2206:int;
      
      protected var var_1389:int;
      
      protected var var_1388:int;
      
      protected var var_3078:uint = 0;
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         ...
         {
            param1.var_45.load(param1.var_260,param1._context);
         }
      }
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1249)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_745[_loc2_] as LibraryLoader;
               if(_loc1_ && (_loc1_.ready || _loc1_.disposed))
               {
                  var_745.splice(_loc2_,1);
               }
               _loc2_--;
            }
            while(var_745.length < const_1816 && false)
            {
               _loc1_ = var_926.shift() as LibraryLoader;
               if(!_loc1_.ready && !_loc1_.disposed)
               {
                  var_745.push(_loc1_);
                  _loc1_.var_45.load(_loc1_.var_260,_loc1_._context);
               }
            }
         }
      }
      ...
      {
         return !!this.var_260 ? this.var_260.url : null;
      }
      ...
      {
         return !!this.var_1124 ? uint(this.var_1389 - this.var_2206) : uint(getTimer() - this.var_2206);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_422,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_45.close();
            }
      ...
      {
         this.var_260 = param1;
         this._name = parseNameFromUrl(this.var_260.url);
         this.var_1124 = false;
      ...
      {
         if(this.var_1123 && !_disposed)
         {
            this.var_1123 = false;
            if(!this.var_1124 && this.var_260)
            {
               this.var_1389 = -1;
      ...
      {
         if(!this.var_1124 && !_disposed && this.var_1388 > 0)
         {
            try
            {
               this.var_45.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_260,const_1882 - this.var_1388);
            if(this.var_2995)
            {
               this.var_260.requestHeaders.push(new URLRequestHeader("pragma","no-cache"));
               this.var_260.requestHeaders.push(new URLRequestHeader("Cache-Control","no-cache"));
               this.var_2995 = false;
            }
            --this.var_1388;
            this.var_501 = 0;
            this.var_45.load(this.var_260,this._context);
            return true;
         }
         return false;
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               ...
               this.debug("Load event STATUS " + this.var_422 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_422,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_422,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_422))
               {
                  this.var_1389 = getTimer();
            ...
            case TimerEvent.TIMER:
               ...
               if(!_disposed)
               {
                  this.var_501 = LibraryLoader.name_5;
         ...
         if(this.var_501 == LibraryLoader.const_2016)
         {
         ...
         if(this.var_501 == LibraryLoader.name_5)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_422,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_652 = this.getDefinition(this._name) as Class;
         if(this.var_652 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_260.url + "!");
            }
            return false;
         }
         ...
         try
         {
            xmlClass = this.var_652.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_422,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_422,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
